...
// [Eluna 现代日志翻译官]：拦截老式 %s，翻译为现代 TC 格式！
#define ELUNA_LOG_INFO(...)  TC_LOG_INFO("server.loading", "{}", fmt::sprintf(__VA_ARGS__)); 
#define ELUNA_LOG_ERROR(...) TC_LOG_ERROR("server.loading", "{}", fmt::sprintf(__VA_ARGS__)); 
#define ELUNA_LOG_FATAL(...) TC_LOG_FATAL("server.loading", "{}", fmt::sprintf(__VA_ARGS__)); 
#define ELUNA_LOG_DEBUG(...) TC_LOG_DEBUG("server.loading", "{}", fmt::sprintf(__VA_ARGS__));
