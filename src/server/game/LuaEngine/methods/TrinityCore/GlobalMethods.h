...
int PerformIngameSpawn(Eluna* E)
    {
        ...
        // [完美匹配 SaveToDB]: 构建动态数组
        std::vector<Difficulty> spawnDifficulties;
        spawnDifficulties.push_back(Difficulty(map->GetDifficultyID()));

        if (spawntype == 1) // spawn creature (召唤怪物)
        {
            if (save)
            {
                Creature* creature = new Creature();
                uint32 spawnId = sObjectMgr->GenerateCreatureSpawnId(); 

                // 完美匹配 Creature::Create 的 9 个参数
                if (!creature->Create(spawnId, map, entry, x, y, z, o, nullptr, 0)) 
                {
                    delete creature;
                    E->Push();
                    return 1;
                ...
                creature->SaveToDB(map->GetId(), spawnDifficulties);
        ...
        if (spawntype == 2) // Spawn object (召唤游戏物体)
        {
            ...
            // [终极破局点]：使用 public static 工厂方法直接创建！不需要 new GameObject()！
            GameObject* object = GameObject::CreateGameObject(objectInfo->entry, map, pos, rot, 255, GO_STATE_READY, 0);
            ...
            if (save)
            {
                object->SaveToDB(map->GetId(), spawnDifficulties);
                uint32 spawnId = object->GetSpawnId();

                delete object;

                object = new GameObject();
                if (!object->LoadFromDB(spawnId, map))
                {
                    delete object;
                    E->Push();
                    return 1;
    ...
    int Ban(Eluna* E)
    {
        ...
        result = BanReturn::BAN_NOTFOUND; // 强行返回找不到，让下游顺畅执行
        switch (result)
        {
