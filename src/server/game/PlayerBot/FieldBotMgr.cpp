...
void WorldPoster::InitializePoster()
{
	...
	PushPoster("      ʹ õ    Ա    ̡   ӱ Ƽ ħ  ֱӪ ꡿       Ʒ");
	PushPoster(" Ա    ӱ Ƽ 88     Ʒֻ   Ű   ϣս  ");
	PushPoster(" Ա    ӱ Ƽ 88     Ʒֻ    սʿ    ʿ    ʦ    ְҵ");
	PushPoster(" Ա    ӱ Ƽ 88     Ʒ      Ұ    սϵͳ");
	PushPoster(" Ա    ӱ Ƽ 88     Ʒ            ³ ϵͳ");
	PushPoster(" Ա    ӱ Ƽ 88     Ʒ     ž     ");
	PushPoster(" Ա    ӱ Ƽ 88     Ʒ ޷  л  Ŷӣ  ޷      ŶӸ   ");
	PushPoster("     ϲ     ǵĲ Ʒ   빺        ");
	PushPoster("    ʽ     Ա   ϵ        ӱ Ƽ 88      QQ  ϵ277922486");
	PushPoster("     κ   ϵ  ʽ      ƭ   ");
}
		...
		if (nearPlayer->IsAlive() && nearPlayer->IsPlayerBot())
		{
			...
			if (BotFieldAI* pFieldAI = dynamic_cast<BotFieldAI*>(nearPlayer->GetAI()))
			{
				if (nearPlayer->GetTeamId() == weaknessTeam)
				{
                    ...
                    Position pos = GetEvadePosition(targetPlayer, nearPlayer);
					if (!pFieldAI->HasCruxMovement())
						pFieldAI->SetCruxMovement(pos);
				}
